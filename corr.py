...
SLIDING_PERIOD = WINDOW_RANGE/2#SAMPLE_FREQUENCY
...
def parse_input_data(filename, ref = 'None'):
    ...
    def calculate_corr_ref(data, data_type = 'None'):
        ...
        print corr_ref
        return corr_ref
    def calculate_corr_change(data, ref_mean, ref_std, data_type = 'none'):
        corr_change = []
        for i in range(int(START_TIME*SAMPLE_FREQUENCY), int(END_TIME*SAMPLE_FREQUENCY), SLIDING_PERIOD):
            if (data_type == 'Time'):
                data_correlation = transforms.TimeCorrelation_whole(50, 'usf').apply(data[:, i:i+WINDOW_RANGE])
            elif (data_type == 'Frequency'):
                data_correlation = transforms.FreqCorrelation_whole(1, 50, 'usf').apply(data[:, i:i+WINDOW_RANGE])
            if i == START_TIME*SAMPLE_FREQUENCY:
    ...
    t_corr_ref = calculate_corr_ref(ref_data, data_type = "Time")
    start = report_time(start)
    t_corr_ref_std = transforms.Stats().apply(t_corr_ref)
    print 'ref std:'
    print t_corr_ref_std
    start = report_time(start)
    t_corr_ref_mean = np.average(t_corr_ref, axis = 1)
    print 'ref avg:'
    print t_corr_ref_mean
    start = report_time(start)
    t_corr_change = calculate_corr_change(data, t_corr_ref_mean, t_corr_ref_std, data_type = 'Time')
    print "Saving corr..."
    current_path = os.path.dirname(os.path.abspath(__file__))
    savefilename = os.path.join(current_path, 'out/0803_corr_change_%ds_%ds'%(START_TIME, END_TIME))
    scipy.io.savemat(savefilename, {'corr':t_corr_change})
    ...
    plot_figures(latencies = get_data(mat_data, 'latencies'), seizure_num_by_slope = slope_num,
           # slope_change = slope_change,
            t_eigen_change = t_corr_change, f_eigen_change = [] )
    """
    plot_figures(latencies = get_data(mat_data, 'latencies'), seizure_num_by_slope = slope_num,
            slope_change = slope_change)
